...
zz.views.FEBase = function( dataset_template, datarow_template ){
	...
	this.dataset_soy_ = zz.views.templates.default.dataset;
	this.datarow_soy_ = zz.views.templates.default.datarow;
	...
	/**
	 * Number view-model formatter.
	 * @type {zz.i18n.services.DecimalFormatter}
	 * @private
	 */
	this.number_formatter_ = zz.i18n.services.DecimalFormatter.getInstance( );
};
...
/**
 * Number view-model formatter setter.
 * @param {*} formatter
 */
zz.views.FEBase.prototype.setNumberFormatter = function( formatter ){

	this.number_formatter_ = formatter;
};
...
zz.views.FEBase.prototype.getFieldValue = function( element ){
	...
	var field = this.getFieldByUid( uid );
	var type = this.ram_.get( uid ).controller.getModel( ).getDatarowSchema( )[ field ].type;
	if( type === zz.models.enums.FieldType.STRING ){
	...
	}else if( type === zz.models.enums.FieldType.NUMBER ){

		return this.number_formatter_.parse( goog.isDef( element.value ) ?

			element.value :
			goog.dom.getTextContent( element ) );

	}else if( type === zz.models.enums.FieldType.BOOLEAN ){
...
zz.views.FEBase.prototype.setFieldValue = function( element, value ){
	...
	if( type === zz.models.enums.FieldType.STRING ){
		...
		if( element.value ){
			...
			element.value = this.number_formatter_.format( value );
			...
			goog.dom.setTextContent(
				...
				element,
				this.number_formatter_.format( value ) );
		}
	}else if( type === zz.models.enums.FieldType.BOOLEAN ){
